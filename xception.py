...
writer = SummaryWriter(log_dir='runs/XceptionUnet')
model = XceptionUnet(3, 4).cuda()
show_net_structure(model)
input = np.zeros((2, 3, 640, 640))
print(input.shape)
writer.add_graph(model, torch.tensor(input).to('cuda:0', dtype=torch.float32))
writer.close()
